...
async def update_user_profile(user_id, request, db):
    ...
    images = [{"id": key.split('-',1)[1], "image": value} for key, value in data.items() if key.startswith('img')]
    print('18\n')
    if images:
        print('19\n')
        ...
        for image in images:
            ...
            image_exists = db.query(UserGallery).filter_by(id=image['id']).first()
            if image_exists:
                image_exists.image = destination
            else:
                user.gallery.append(UserGallery(id=image['id'], image=destination))
